class BothDetailsController < ApplicationController
  ...
  def create
    seldet = if current_user.sel_detail
               current_user.sel_detail.update(shipping_adress: params[:shipping_adress],
                                              product_type: params[:sel_detail]['product_type'])
             else
               current_user.create_sel_detail(shipping_adress: params[:shipping_adress],
                                              product_type: params[:sel_detail]['product_type'])
             end
